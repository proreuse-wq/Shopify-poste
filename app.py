...
def crea_spedizione_poste(ordine, paperless=False):
    try:
        ...
        receiver = {
            ...
            "phone": shipping.get("phone", "") or MITTENTE.get("phone", ""),
            "cellphone": "",
            "note1": "",
            "note2": ""
        }
        ...
        if paese == "IT":
            ...
            description = (ordine.get("line_items", [{}])[0].get("title") or "Merce varia")[:30]
            quantity = int((ordine.get("line_items", [{}])[0].get("quantity") or 1))
            item_price = ordine.get("line_items", [{}])[0].get("price", "10")
            try:
                total_value = str(int(round(float(str(item_price).replace(",", ".")) * 100)) * quantity)
            except Exception:
                total_value = "1000"

            data_block["description"] = description
            data_block["packagingCode"] = os.environ.get("POSTE_PACKAGING_CODE", "C")
            data_block["items"] = [{
                "itemNumber": "1",
                "taric": os.environ.get("POSTE_DEFAULT_TARIC", "2313200310"),
                "totalValue": total_value,
                "quantity": str(quantity),
                "totalWeight": str(peso_kg * 1000),
                "originCountry": os.environ.get("POSTE_ORIGIN_COUNTRY_ISO2", "IT")
            }]
            data_block["international"] = {
                "receiverType": os.environ.get("POSTE_DEFAULT_RECEIVER_TYPE", "retailDelivery"),
                "contentCode": os.environ.get("POSTE_INTL_CONTENT_CODE", "999")
            }
        ...
        print("DEBUG MITTENTE PHONE:", MITTENTE.get("phone"))
        ...
        result_info = result.get("result", {}) or {}
        error_code = result_info.get("errorCode")
        if error_code not in (0, "0", None):
            raise RuntimeError(result_info.get("errorDescription") or f"Errore Poste {error_code}")

        ldv = ""
        waybills = result.get("waybills") or []
        if waybills and isinstance(waybills, list):
            first = waybills[0] or {}
            ldv = first.get("code") or first.get("waybill") or first.get("barcode") or first.get("trackingNumber") or ""
